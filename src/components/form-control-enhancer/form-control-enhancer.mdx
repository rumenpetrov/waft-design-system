...
<Meta title="Web/Labs/FormControlEnhancer/Extra" />
...
<table style={{ width: '100%' }}>
  ...
  <tbody>
    {componentTokensToWebTokens((tokenName) => tokenName.startsWith('WDS-INPUT-ENHANCER--')).map(({ webTokenName, webTokenValue, mappedWebTokenName }) => (
      <tr key={webTokenName}>
        <td>{webTokenName}</td>
        <td>{webTokenValue}</td>
